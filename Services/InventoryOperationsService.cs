...
VALUES(@n,@w,N'COMPLETED',@notes,@u,SYSUTCDATETIME()); SELECT CAST(SCOPE_IDENTITY() AS INT);", conn, tx))
                    ...
                    InsertInventoryTransaction(conn,tx,productId,"STOCK_COUNT",Convert.ToInt32(Math.Round(actualQty-expected,0)),oldLegacy,newLegacy,"STOCK_COUNT",countId,userId,warehouseId,reason);
...
VALUES(@n,@f,@t,N'COMPLETED',@notes,@u,SYSUTCDATETIME());SELECT CAST(SCOPE_IDENTITY() AS INT);",conn,tx)) { cmd.Parameters.AddWithValue("@n",number);cmd.Parameters.AddWithValue("@f",fromWarehouseId);cmd.Parameters.AddWithValue("@t",toWarehouseId);cmd.Parameters.AddWithValue("@notes",string.IsNullOrWhiteSpace(notes)?(object)DBNull.Value:notes);cmd.Parameters.AddWithValue("@u",userId);id=Convert.ToInt32(cmd.ExecuteScalar()); }
                    ...
                    InsertInventoryTransaction(conn,tx,productId,"TRANSFER_OUT",-Convert.ToInt32(Math.Round(qty,0)),legacy,legacy,"STOCK_TRANSFER",id,userId,fromWarehouseId,notes);
                    InsertInventoryTransaction(conn,tx,productId,"TRANSFER_IN",Convert.ToInt32(Math.Round(qty,0)),legacy,legacy,"STOCK_TRANSFER",id,userId,toWarehouseId,notes);
                    tx.Commit();return id;
                } catch { tx.Rollback(); throw; }
            }
        }
...
ELSE INSERT dbo.ProductBatches(product_id,warehouse_id,batch_number,expiry_date,quantity,unit_cost) VALUES(@p,@w,@b,@e,@q,@cost);",conn))
        ...
        private void InsertInventoryTransaction(SqlConnection c,SqlTransaction tx,int p,string type,int change,int oldQ,int newQ,string refType,int refId,int user,int warehouse,string notes){using(var cmd=new SqlCommand(@"INSERT dbo.InventoryTransactions(product_id,transaction_type,quantity_change,old_quantity,new_quantity,reference_type,reference_id,user_id,notes,created_at,warehouse_id) VALUES(@p,@t,@chg,@old,@new,@rt,@rid,@u,@notes,SYSUTCDATETIME(),@w);",c,tx)){cmd.Parameters.AddWithValue("@p",p);cmd.Parameters.AddWithValue("@t",type);cmd.Parameters.AddWithValue("@chg",change);cmd.Parameters.AddWithValue("@old",oldQ);cmd.Parameters.AddWithValue("@new",newQ);cmd.Parameters.AddWithValue("@rt",refType);cmd.Parameters.AddWithValue("@rid",refId);cmd.Parameters.AddWithValue("@u",user);cmd.Parameters.AddWithValue("@notes",string.IsNullOrWhiteSpace(notes)?(object)DBNull.Value:notes);cmd.Parameters.AddWithValue("@w",warehouse);cmd.ExecuteNonQuery();}}
