...
namespace Pos_System.Forms
{
    internal sealed class SalesLifecycleForm : Form
    {
        ...
        private TabPage BuildQuotationTab(){var page=NewPage("Quotations");var bar=NewBar();var refresh=Button("Refresh",12);refresh.Click+=(s,e)=>LoadQuotations();bar.Controls.Add(refresh);page.Controls.Add(quotationGrid);page.Controls.Add(bar);return page;}
        ...
        private void ResumeSelected(){try{if(heldGrid.CurrentRow==null)return;int id=Convert.ToInt32(heldGrid.CurrentRow.Cells["held_sale_id"].Value);List<SalesLifecycleService.CartLine> lines=service.LoadHeldItems(id);if(lines.Count==0){MessageBox.Show("This held sale has no items.");return;}SalesAdvancedUiService.QueueResume(id,lines);using(var sales=new Sales(LoginForm.LoggedInUsername,AppSession.Role))sales.ShowDialog(this);LoadHeld();}catch(Exception ex){Handle(ex,"RESUME_SALE");}}
        ...
        private void CompleteReturn()
        {
            try
            {
                ...
                int id=service.ProcessReturn(saleId,AppSession.UserId,Convert.ToString(returnType.SelectedItem),Convert.ToString(refundMethod.SelectedItem),reasonText.Text,lines);MessageBox.Show("Return completed. Return ID: "+id,"Return",MessageBoxButtons.OK,MessageBoxIcon.Information);LoadReturnItems();
            }catch(Exception ex){Handle(ex,"COMPLETE_RETURN");}
        }

        private void RefreshAll(){LoadHeld();LoadQuotations();}
        private void LoadHeld(){try{heldGrid.DataSource=service.GetHeldSales();}catch(Exception ex){Handle(ex,"LOAD_HELD");}}
        private void LoadQuotations(){try{quotationGrid.DataSource=service.GetQuotations();}catch(Exception ex){Handle(ex,"LOAD_QUOTATIONS");}}
        private static void Handle(Exception ex,string op){ErrorLogService.Log(ex,op,nameof(SalesLifecycleForm));MessageBox.Show(ex.Message,"Sales Operations",MessageBoxButtons.OK,MessageBoxIcon.Error);}
