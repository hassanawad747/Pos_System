...
namespace Pos_System.Forms
{
    internal sealed class SecurityAdminForm : Form
    {
        ...
        {
            try{ActionPermissionService.Demand("USER.PERMISSION_EDIT");if(permissionsGrid.CurrentRow==null)return;int id=Convert.ToInt32(permissionsGrid.CurrentRow.Cells["action_permission_id"].Value);bool allowed=Convert.ToBoolean(permissionsGrid.CurrentRow.Cells["is_allowed"].Value);using(var c=Open())using(var cmd=new SqlCommand("UPDATE dbo.ActionPermissions SET is_allowed=@a,updated_by=@u,updated_at=SYSUTCDATETIME() WHERE action_permission_id=@id;",c)){cmd.Parameters.AddWithValue("@a",allowed);cmd.Parameters.AddWithValue("@u",AppSession.UserId);cmd.Parameters.AddWithValue("@id",id);cmd.ExecuteNonQuery();}MessageBox.Show("Permission saved.");}catch(Exception ex){ShowError(ex);}
        }
