...
INSERT dbo.LoyaltyAccounts(customer_id,points_balance,lifetime_points) SELECT customer_id,5,5 FROM dbo.Customers;");
        ...
        private static void PermissionTests(){AppSession.Set(new User{User_Id=userId,Username="cashier_test",Role="cashier"});Check(ActionPermissionService.Can("SALE.CREATE",Cs)&&!ActionPermissionService.Can("STOCK.TRANSFER",Cs),"sensitive action permissions");AppSession.Set(new User{User_Id=userId,Username="admin",Role="admin"});}
